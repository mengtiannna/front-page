...
export default class GoodsListView extends React.Component<any, any> {
  constructor(props) {
    super(props);
    this.state = {
      items: Array.from(Array(5000), () => ({
        id: Math.random().toFixed(6),
        name: Math.random().toFixed(6),
        src: catNames[(Math.random() * 100).toFixed(0)],
      })),
    };
  }
...
class FakeCard extends Component {
  render() {
    ...
    return (
      <div className="card">
        <img className="img" alt="kitty" src={src} width={170} />
        <span>{name}</span>
      </div>
    );
  }
